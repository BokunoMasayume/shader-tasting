...
#define _saturat(x) clamp(x, 0.0, 1.0)
...
{
    vec3  ab = b - a;
    float t  = _saturat(dot(p - a, ab) / dot(ab, ab));
...
vec3 wavelengthToRGB(float wavelength)
{
    ...
    vec3 colour = oz.xxx - _saturat(vec3(abs(wavelength-kLambdaR), abs(wavelength-kLambdaG), abs(wavelength-kLambdaB))/150.0);
	return colour;  
}
...
float fMapleLeaf(vec3 posLeaf, float scale, float rand, bool doDetail, out vec4 material)
{
    ...
    float normalisedHeight = _saturat(0.5 + (posLeaf.y/scale));
    ...
    if(doDetail)
    {
        pointyBits = abs(fract((-0.08 + posTri.y + abs(posTri.x)*2.0)*8.0) - 0.5) * 0.1 
            * _saturat(1.0 - abs(-posTri.y/scale + 0.4)*1.);
    ...
    {    
        pointyBits = abs(fract((-0.03 + posTri.y + abs(posTri.x)*2.0)*8.0) - 0.5) * 0.1 
            * _saturat(1.0 - abs(-posTri.y/scale + 0.35)*2.5);
    ...
    if(doDetail)
    {
        ...
        float stickProgress = _saturat(-posStick.y/scale);
...
float fSmallBranchesSDF(vec3 posWS, float branchDist, float branchProgress, out vec4 material)
{
    ...
    minDist += _saturat(0.7 - branchProgress);
    ...
    minDist += _saturat((branchProgress - 1.4)*3.0);
...
float fGrassBladeSet(vec3 grassPosWS, float iter, float scale, float flattenAmount, inout vec4 material)
{
    ...
    float normalisedHeight = _saturat(grassPosWS.y / height);
...
{    
    ...
    {
        minDist = groundY;
    	material = vec4(kMatGrass, 0.0, _saturat(grassDist / 0.2) * 0.3, 0.0);
...
vec3 getSky(vec3 rayDirWS, float roughness, out vec3 transmittance)
{
	...
	vec3 sunTransmittance = max(0.00025*oz.xxx, //Max to prevent the sky from being pitch black at night
		earthShadow * mix(rayleighTransmittance, oz.xxx, 0.2 + 0.8*_saturat((s_dirToSunWS.y)*3.0)));
